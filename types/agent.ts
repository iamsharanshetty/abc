...
=======
  if (
    settings.summary !== undefined &&
    settings.summary.length > c.summary.maxLength
  ) {
>>>>>>> chat-backup
...
=======
  if (
    ...
    settings.customInstructions.length > c.customInstructions.maxLength
  ) {
    errors.push(
      `Custom instructions cannot exceed ${c.customInstructions.maxLength} characters`
    );
>>>>>>> chat-backup
  }
